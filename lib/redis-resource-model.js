...
module.exports = oo.createClass(events, {
	...
	_create: function(config){
		...
		for(let i =0, ln =this.associations.length, item; i<ln; i++){
			...
			this.associations[i] = new ResourceModelAssociation(item);
		}
	},
	...
	create: function(env, next) {
		...
		co(function *(){
			...
			yield self.spi.executeMultipleOperations(multi);

			return resource;
		})(function(err, result){
			if (err || !result) {
				// TODO: Write a more pro error piping solution
				respond.sendErrorResponse(err, env, next);
			} else {
	...
	_checkIndexes: thunkify(function(client, resource, cb){
		...
		client = client.hasOwnProperty('_client') ? self.spi.getClient() : client;
		...
		co(function *(){
			return yield parallel(self.indexes.map(function *(index){
				...
				if(result){
					throw new ApiError(422, 'Duplicate ' + index + ': ' + resource[index] + ' for ' + self.resourceType);
				}
		...
		co(function *(){
			yield parallel(required.map(function *(key){
				if(!resource.hasOwnProperty(key)){
					throw new ApiError(422, 'Missing required key [' + key + ']');
				}
				return true;
			}));
	...
	_createAssociations: thunkify(function(client, resource, cb){
		...
		co(function *(){
			yield parallel(self.associations.map(function *(association){
				return yield association.create(client, resource);
			}));
	...
	_deleteAssociations: thunkify(function(client, resource, cb){
		...
		co(function *(){
			yield parallel(self.associations.map(function *(association){
				return yield association.remove(client, resource);
			}));
	...
	validate: thunkify(function(resource, cb){
		...
		co(function *(){
			...
			yield parallel(Object.keys(resource).map(function *(key){
				...
				if(!field){
					throw new ApiError(422, 'Property ' + key + ' is not allowed in model');
				}
					...
					yield parallel(validations.map(function *(validation){
						if (validation.hasOwnProperty('fn')) {
							if (!validation.fn(resource[key])) {
								throw new ApiError(422, 'Validation Error:' + validation.err);
							}
						}
						return true;
					}));
	...
	get: function(env, next) {
		...
		co(function *(){
			...
			if(!uuid){
				throw new ApiError(422, 'ID not present in request');
			}

			return yield self.spi.getResource(self.spi.getClient(), uuid, self.resourceType);
	...
	getResourcesInForeignSet: function(){
	...
	update: function(env, next) {
		...
		co(function *(){
			...
			resource = yield self.spi.updateResource(multi, env.route.params.id, resource, self);
			...
			if(err){
				...
				respond.sendUpdateEntityResponse(result, env, next);
			}
		});
	},
	...
	remove: function(env, next) {
		...
		co(function *(){
			...
			if(!resourceId){
				throw new ApiError(422, 'Missing ResourceID for DELETE');
			}
			...
			if(err){
				...
				respond.sendDeleteEntityResponse(resourceId, env, next);
			}
		});
	}
});