...
var ResourceModel = module.exports = oo.createClass({
	...
	_create: function(config){
		...
		for(var i =0, ln =this.associations.length; i<ln; i++){
			this.associations[i] = ResourceModelAssociation(this.associations[i]);
		}
	},
			...
			if (self.properties){
				for(key in self.properties){
					...
					if(typeof field == 'function'){
						obj[key] = field.call(self, obj[key]||null);
					}
				}
			}

			self._createInternal(self, obj, env, next);
		});
	},

	_createInternal: function(self, resource, env, next){
	...
	getByIndex: function(env, next) {
		var key = env.route.params.key,
			value = env.route.params.value;

		this.spi.getResourceByIndex(this.spi.getClient(), this, key, value, function(err, resource){
			if(err || !resource){
				respond.sendEntityNotFoundResponse(undefined, env, next);
			} else {
				respond.sendEntityFoundResponse(resource, env, next);
