...
var ResourceModel = module.exports = oo.createClass({
	...
	create: function(env, next) {
		...
		let self = this,
				client = self.spi.getClient().multi();

		co(function *(){

			let body, resource;

			body = yield parseBody.json(env.request);

			// Run field converters
			body = yield thunkify(self.generateFields).call(self, body);

			// Validate
			yield thunkify(self.validate).call(self, body);

			// Create resource
			resource = yield thunkify(self.spi.createResource).call(self, body, self);

			// Create indexes
			if(self.indexes && self.indexes.length){
				yield thunkify(self._checkIndexes).call(self, resource);
				yield thunkify(self._createIndexes).call(self, client, resource);
			}

			// Create Associations
			if (self.associations && self.associations.length){
				resource = yield thunkify(self._createAssociations).call(self, client, resource);
			}

			// Add to Sets Associations
			if (self.sets && self.sets.length){
				resource = yield thunkify(self._addToSets).call(self, client, resource);
			}
			...
			if(self.afterCreateResource && typeof self.afterCreateResource === 'function'){
				resource =  yield thunkify(self.afterCreateResource).call(self, self.spi.getClient(), resource);
			}
	...
	_addToSets: function(client, resource, cb){
		...
		async.each(self.sets, function(item, callback){
			self.spi._addToSet(client, item, resource[self.primary]);
	...
	_createIndexes: function(client, resource, cb){
		...
		async.each(self.indexes, function(item, callback){
			self.createSecondaryIndexForKey(client, self.resourceType, item, resource, self.primary);
			callback(null, resource);
		}, function(err){
			cb(err, resource)
		});
	...
	_checkIndexes: function(client, resource, cb){
		...
		async.each(self.indexes, function(index, callback) {
			if (!resource[index]) {
				callback('Missing index [' + index + ']');
			} else {
				self._indexExists(client, self.resourceType, index, resource[index], function (err, result) {
					if (err||!result) {
						callback(err||'Duplicate index for resource');
					} else {
	...
	_createAssociations: function(client, resource, cb){
		...
		async.each(self.associations, function(association, callback){
