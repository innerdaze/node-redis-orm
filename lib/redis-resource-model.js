...
ResourceModel.prototype.list = function(env, next) {
	env.response.body = this.storageApi.getResourcesInSet(this.resourceType + 's', function(err, replies){
...
function createInternal(self, resource, env, next){
	...
	self.storageApi.createResource(resource, self, function(err, resource){
		if(err){
			respond.sendBodyErrorResponse(err, env, next);
		} else {
			...
			if(self.afterCreateResource && typeof self.afterCreateResource === 'function'){
				self.afterCreateResource(self.storageApi.client, resource, function(err, resource){
					if(err){
						respond.sendBodyErrorResponse(err, env, next);
					} else {
...
ResourceModel.prototype.get = function(env, next) {
	...
	this.storageApi.getResource(uuid, this, function(err, resource){
		if(err || !resource){
...
ResourceModel.prototype.getByIndex = function(env, next) {
	...
	this.storageApi.getResourceByIndex(this, key, value, function(err, resource){
		if(err || !resource){
			respond.sendEntityNotFoundResponse(undefined, env, next);
		} else {
			respond.sendEntityFoundResponse(resource, env, next);
...
ResourceModel.prototype.update = function(env, next) {
	...
	env.request.getBody(function(err, body) {
		...
		self.storageApi.updateResource(obj, self, function(err, resource){
			console.log('Created ' + self.resourceType);
...
ResourceModel.prototype.remove = function(env, next) {
	...
	if(self.beforeDeleteResource && typeof self.beforeDeleteResource === 'function'){
		...
		self.storageApi.getResource(uuid, self, function(err, resource){

			if(err){
				respond.sendEntityNotFoundResponse('Could not find resource to delete: ' + uuid, env, next);
			} else {

				self.beforeDeleteResource(self.storageApi.client, resource, function(err, resource){
					if(err){
						respond.sendBodyErrorResponse(err, env, next);
					} else {
						self.storageApi.deleteResource(uuid, self, function(err, deletedCount){
