...
var ResourceModel = module.exports = oo.createClass({
	...
	create: function(env, next) {
		...
		co(function *(){
			...
			var body = yield parseBody.json(env.request);
			...
			if (self.properties){
				for(key in self.properties){
					...
					if(field.type && typeof field.type == 'function'){
						body[key] = field.type.call(self, body[key]||null);
					}
				}
			}

			var valid = yield thunkify(self.validate.bind(self))(body);

			if(valid){

				var resource = yield thunkify(self.spi.createResource.bind(self))(body, self)
				...
				if(resource){
					...
					if(self.afterCreateResource && typeof self.afterCreateResource === 'function'){
						...
						return yield thunkify(self.afterCreateResource.bind(self))(self.spi.getClient(), resource);
