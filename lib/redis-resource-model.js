...
module.exports = oo.createClass(events, {
	...
	get: function(env, next) {
		...
		co(function *(){
			...
			return yield thunkify(self.spi.getResource).call(self.spi, uuid, this.spi.getClient(), this.resourceType);
	...
	getBySecondaryIndex: function(client, env, next) {
		let self = this;
		...
		co(function *(){
			...
			return yield thunkify(self.spi.getKeyBySecondaryIndex).call(self, client, self, key, value);
		})(function(err, resource){
			if(err || !resource){
				respond.sendEntityNotFoundResponse(undefined, env, next);
			} else {
