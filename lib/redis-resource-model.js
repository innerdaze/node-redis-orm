...
module.exports = oo.createClass(events, {
	...
	get: function(env, next) {
		...
		co(function *(){
			...
			return yield thunkify(self.spi.getResource).call(self.spi, uuid, self.spi.getClient(), this.resourceType);
	...
	getBySecondaryIndex: function(env, next) {
		...
		co(function *(){
			...
			return yield thunkify(self.spi.getKeyBySecondaryIndex).call(self, self.spi.getClient(), self, key, value);
		})(function(err, resource){
			if(err || !resource){
				respond.sendEntityNotFoundResponse(undefined, env, next);
			} else {
	...
	update: function(env, next) {
		...
		co(function *(){
			...
			resource = yield thunkify(self.spi.updateResource).call(self.spi, multi, resource, self);
