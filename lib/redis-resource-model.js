...
module.exports = oo.createClass(events, {
	...
	_checkIndexes: thunkify(function(client, resource, cb){
		...
		co(function *(){
			return yield parallel(self.indexes.map(function *(index){
				...
				if(result){
					new ApiError(422, 'Duplicate ' + index + ': ' + resource[index] + ' for ' + self.resourceType);
				}
	...
	validate: thunkify(function(resource, cb){
		...
		co(function *(){
			...
			yield parallel(Object.keys(resource).map(function *(key){
				...
				if(!field){
					throw 'Property ' + key + ' is not allowed in model';
				}
	...
	get: function(env, next) {
		...
		co(function *(){
			...
			let result = yield self.spi.getResource(self.spi.getClient(), uuid, self.resourceType);
	...
	remove: function(env, next) {
		...
		co(function *(){
			...
			if(!resourceId){
				throw 'Missing ResourceID for delete. Cannot continue';
			}
