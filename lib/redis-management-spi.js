...
var RedisManagementSpi = oo.createClass({
	...
	_create: function(config){
		...
		if(!this.getClient()){
			...
			this.setClient(redis.createClient(port, host, ropts));
		}
	},
	 ...
	 * @param cb
	 */
	getResourceByIndex: function(client, config, index, value, cb){
		var self = this;

		self.getKeyForIndex(client, config, index, value, function(err, resourceId){
			if (!resourceId) {
