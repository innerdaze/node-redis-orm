...
//		}
			...
			self.getResource(resourceId, config, function(err, resource){
				if(err){
					...
					for (i = 0, ln = config.indexes.length; i < ln; i++) {
						...
						self._deleteIndexForKey(multi, config, indexKey, resource[indexKey]);
					}
					multi.exec(cb);
				}
			});
	...
	createSecondaryIndexForKey: function(client, config, key, resource, resourceKey, cb) {
		var keyArray = [config.resourceType].concat(key).concat(resource[key]);
		client.hset(this._key.apply(this, keyArray), resource[resourceKey], resource[config.primary], cb || function(){});
	},
	 ...
	 * @param cb
	 */
	getResourceBySecondaryIndex: function(client, config, index, value, cb){
		var self = this;
