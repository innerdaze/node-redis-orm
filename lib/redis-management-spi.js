...
module.exports = oo.createClass({
	 ...
	 * @param cb
	 */
	updateResource: thunkify(function (client, resource, config, cb) {
		let self = this;
	 ...
	 * @param cb
	 */
	deleteResource: thunkify(function(resourceId, config, cb){
		...
		co(function *(){
			...
			if(config.sets){
				yield parallel(config.sets.map(function *(name){
					return yield self.deleteFromSet(multi, config, name, resourceId);
				}));
			}

			// - Remove from indexes
			if(config.indexes){
				let resource = yield self.getResource(client, resourceId, config.resourceType);
...
//	},
	 ...
	 */
	createSecondaryIndexForKey: thunkify(function(client, resourceType, key, resource, resourceKey, cb) {
		let self = this;
	 ...
	 * @param cb
	 */
	executeMultipleOperations: thunkify(function(multi, resource, cb){
		co(function *(){
