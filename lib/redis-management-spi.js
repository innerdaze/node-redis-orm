...
//		}
			...
			self.getResource(resourceId, config, function(err, resource){
				if(err){
					...
					for (i = 0, ln = config.indexes.length; i < ln; i++) {
						...
						self.deleteSecondaryIndexForKey(multi, config, indexKey, resource[indexKey]);
					}
					multi.exec(cb);
				}
			});
	 ...
	 */
	createSecondaryIndexForKey: function(client, config, key, resource, resourceKey, cb) {
		var keyArray = [config.resourceType].concat(key);
		client.hset(this._key.apply(this, keyArray), resource[key], resource[resourceKey], cb || function(){});
	},
	 ...
	 * @param {Function} [cb]
	 */
	deleteSecondaryIndexForKey: function(client, config, key, value, cb) {
		client.hdel(this._key(config.resourceType, key), value, cb || function(){});
	},
	...
	_createResourceInternal: function(client, resource, config, cb) {
		...
		if (config.indexes) {
			for (i = 0, ln = config.indexes.length; i < ln; i++) {
				indexKey = config.indexes[i];
				self.createSecondaryIndexForKey(multi, config, indexKey, resource, 'id');
			}
		}
