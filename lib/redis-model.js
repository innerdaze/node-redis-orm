...
Model.prototype.list = function(env, next) {
	env.response.body = this.redis.getResourcesInSet(this.storageConfig.resourceType + 's', function(err, replies){
...
Model.prototype.create = function(env, next) {
	...
	env.request.getBody(function(err, body) {
		...
		self.redis.createResource(obj, self.storageConfig, function(err, customer){
			if(err){
				respond.sendBodyErrorResponse(err, env, next);
			} else {
...
Model.prototype.show = function(env, next) {
	...
	this.redis.getResource(uuid, this.storageConfig, function(err, customer){
		if(err || !customer){
...
Model.prototype.update = function(env, next) {
	...
	env.request.getBody(function(err, body) {
		...
		self.redis.updateResource(obj, self.storageConfig, function(err, customer){
			console.log('Created ' + self.storageConfig.resourceType);
...
Model.prototype.remove = function(env, next) {
	...
	this.redis.deleteResource(uuid, this.storageConfig, function(err, deletedCount){
