...
module.exports = oo.createClass({
	...
	callRedisCo: thunkify(function(client, command, args, cb){
		co(function *(){
			if(!client[command]){
				throw new ApiError(500, command + ' is not a method of RedisManagementSPI');
			}
...
//	},
	 ...
	 */
	createSecondaryIndexForKey: thunkify(function(client, resourceType, index, resource, resourceIndexKey, cb) {
		let self = this;

		co(function *(){

			let secondaryIndex, value;

			secondaryIndex = self._key.apply(self, [resourceType].concat(index));
	...
	getKeyBySecondaryIndex: thunkify(function(client, config, index, value, cb){
		...
		co(function *(){
			...
			key = index ? self._key(config.resourceType, index) : self._key(index);

			resourceId = yield self.callRedisCo(self.getClient(), 'hget', [key, value]);

			if(!resourceId){
				throw new ApiError(404, 'No resource found for: ' + index);
			}
