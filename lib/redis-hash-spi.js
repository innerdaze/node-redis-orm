 ...
 */
var KEY_PREFIX = 'namespace:resource';
...
RedisManagementSpi.prototype.deleteResource = function(resourceId, config, cb){
	...
	if(config.associations){
		for(i = 0, ln = config.associations.length; i<ln; i++){
			config.associations.del(multi, resourceId)
		}
	}
...
RedisManagementSpi.prototype.createIndexForKey = function(client, config, key, resource, resourceKey, cb) {
	...
	client.set(_key.apply(this, keyArray), _key(config.resourceType, resource[resourceKey]), cb || function(){});
};
...
RedisManagementSpi.prototype.deleteKeyPartsIndex = function(client, config, key, resource, resourceKey, cb) {
	var keyArray = [config.resourceType].concat(key).concat(resourceKey);
	client.del(_key.apply(this, keyArray), cb || function(){});
};
...
function deleteIndexForKey(client, config, key, value, cb) {
	client.del(_key(config.resourceType, key, value, config.primary), cb || function(){});
}
 ...
 * @param cb
 */
function createResourceInternal(spi, client, resource, config, cb) {
	...
	if (config.indexes) {
		for (i = 0, ln = config.indexes.length; i < ln; i++) {
			indexKey = config.indexes[i];
			spi.createIndexForKey(multi, config, indexKey, resource, 'id');
		}
	}
	...
	if (config.associations){
		async.every(config.associations, function(association, callback){
			if(resource[association.localKey]){
				association.create(spi, client, config, resource, callback);
			}
		}, function(err){
			if(err){
				cb(err);
...
//function createAssociation(spi, client, config, localResource, associationConfig, cb){
//	var foreignId = localResource[associationConfig.localKey];
//
//	client.get(_key(associationConfig.foreignResource, foreignId), function(err, foreignResource){
//		if(err){
